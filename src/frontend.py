...
with st.spinner(text="Preparing Data To Plot"):
    ...
    geo_pred_df = pd.merge(geo_df, predictions_df, left_on='LocationID', right_on='pickup_location_id')
    ...
    geo_pred_df['fill_color'] = geo_pred_df['predicted_demand'].apply(lambda x: pseudocolor(x, min_demand, max_demand, BLACK, ORANGE))
...
with st.spinner(text="Generating NYC Map"):
    ...
    # Set elevation
    geo_pred_df['elevation'] = geo_pred_df['predicted_demand'] * 2  # Adjust the multiplier as necessary
...
with st.spinner(text="Plotting Time-Series Data"):
    ...
    merged_df = pd.merge(features_df, predictions_df, on='pickup_location_id', suffixes=('_features', '_preds'))
    merged_df = merged_df.merge(geo_df[['LocationID', 'zone']], right_on = 'LocationID', left_on = 'pickup_location_id')
    merged_df['predicted_demand'] = np.clip(merged_df['predicted_demand'], 0, None)

    # Sort merged_df by 'predicted_demand' in descending order and get the top 10 rows
    top_10_preds = merged_df.sort_values(by='predicted_demand', ascending=False).head(10)

    # Add download button in the top right corner
    df_to_download = geo_pred_df.copy().drop(['elevation','fill_color', 'LocationID'], axis=1)
    ...
    st.markdown("<div style='text-align: left; font-size: small;'>Note: This data should not be used for \
        operational purposes. Data from the most recent hours are not available. Hence, a travel \
            simulation is performed and used as a reliable basis for making forecasts.\
